...
function opening(target: AgentInstallTarget): string {
  switch (target) {
    case 'codex':
      return '请将本地目录中的 Wemento Reader Skill 安装到当前 Codex 项目或用户 Skill 目录：'
    case 'claude-code':
      return '请安装以下本地 Wemento Reader Skill，并按照 SKILL.md 调用本地 HTTP API：'
    case 'openclaw':
      return '请将以下本地目录作为 Wemento Reader Skill 安装，并阅读其中的 SKILL.md：'
    default:
      return '请读取并安装以下 Wemento Reader Skill：'
  }
}
...
export function buildSkillVerificationPrompt(): string {
  return '请检查 Wemento Reader 是否已连接，然后列出最近 5 个微信会话。'
}
