...
export function ExportWorkspace({
  ...
  const targetRootLabel = outputDirectory || 'Wemento 数据目录/exports'
  const selectedTargetPath = exportAll
    ? format === 'html' && zip
      ? `${targetRootLabel}/${outputName}.zip`
      : `${targetRootLabel}/${outputName}/`
    : format === 'html'
      ? zip
        ? `${targetRootLabel}/${outputName}.zip`
        : `${targetRootLabel}/${outputName}/`
      : `${targetRootLabel}/${outputName}.${format === 'markdown' ? 'md' : format}`
