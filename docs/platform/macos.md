...
Wemento 需要读取微信本地数据。macOS 会根据系统版本、微信状态和安全设置，要求应用完成授权；自动获取数据库密钥时，页面可能提示暂时调整系统安全设置。

## 推荐步骤

1. 先启动 Wemento，阅读连接页面显示的当前前置条件。
2. 确认微信数据目录指向当前账号。
...
关闭 System Integrity Protection 会降低 macOS 对系统文件和进程的保护。它不是日常使用 Wemento 的功能开关，也不应长期保持关闭。只有在你理解风险、确认页面要求且完成必要操作时才处理；完成后按 Apple 官方方式重新启用。
