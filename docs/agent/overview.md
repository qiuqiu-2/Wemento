# 在微信机器人或外部 Agent 中使用 Wemento

Wemento 提供两条不同路径。先按你实际想做的事选择，不需要先理解 Agent、Skill 或 API 等术语。
...
打开应用一级导航中的“Agent”，进入“Agent Hub”，扫码登录一个微信机器人账号。之后用另一个微信账号向机器人发送文字，它会调用 Wemento 的本机数据，必要时使用已配置的 AI，再把结果回复给发送者。
...
Reader Skill 是给外部 Agent 的操作说明。安装后，Codex、Claude Code、OpenClaw 或其他本地 Agent 可以通过 Wemento Local HTTP API 按需读取联系人、群聊、最近会话、指定时间范围的聊天和群成员信息。
