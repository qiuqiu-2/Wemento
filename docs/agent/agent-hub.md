# 在微信里向 Wemento 提问（Agent Hub）

Agent Hub 是 Wemento 内置的微信机器人入口，也是应用一级导航中的“Agent”页面。你先扫码登录一个微信机器人账号，再用微信账号向机器人发送文字；本机 Agent Hub 会接收消息、读取已经连接的微信数据，必要时调用已配置的 AI，再把结果回复给发送者。
...
你不需要单独部署这些组件。扫码后，后台的微信连接器负责登录机器人、保持连接、接收微信消息和发送回复；Agent Hub 负责判断消息要做什么、查询 Wemento 本地数据、调用 AI 并组织结果。可以把它理解为：连接器负责“和微信通信”，Hub 负责“处理任务”。
...
- Wemento 的微信数据库已经连接，并且数据 API 可以查询；
- 依赖总结或自然语言理解的任务，需要在“设置 → AI 模型”配置可用的 AI 服务；
- Wemento 和 Agent Hub 需要保持运行，机器人才能接收和回复消息。
