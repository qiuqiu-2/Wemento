...
1. Wemento 正在运行且 API Center 显示本地服务在线；
...
防撤回只能尽量保留开启后且应用成功捕获到的撤回变化。确认开启时数据库已经连接、Wemento 在撤回发生时保持运行，并检查聊天加载是否明显变慢。开启前已经消失、应用未捕获或微信结构无法识别的消息不能保证恢复；详见[防撤回](./recall-protection.md)。
