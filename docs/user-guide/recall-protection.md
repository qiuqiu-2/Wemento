...
防撤回是一个默认关闭的可选功能。开启后，Wemento 会尽量保留它能够捕获到的撤回消息，并在聊天气泡旁标记“消息已撤回”。
...
2. 打开“设置 → 防撤回”。
3. 阅读性能和数据提示后，开启“防撤回”。
4. 保持 Wemento 与当前微信数据连接；之后捕获到的撤回消息会尽量保留并标记。
...
- 按微信账号隔离 Wemento 保存的恢复记录。

## 当前限制

- 不能恢复开启前已经撤回、且应用从未保存到的消息；
- Wemento 未运行、数据库未连接或没有捕获到撤回变化时，消息可能无法保留；
...
普通浏览、搜索和 Knowledge 不会修改微信原始聊天数据库；防撤回是一个例外。用户第一次明确开启时，当前实现会在微信消息数据库中安装用于记录撤回的本地日志/监听结构，并在 Wemento 的用户数据目录保存必要的本地恢复记录。
